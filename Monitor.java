...
public class Monitor {
	...
	public synchronized void checkActions(int philosopherPos) {
		try {
			while (true) {
				...
				/*
				 * Here we have the (philosopherPos +1 being your left-handed side and your
				 * left-handed side
				 * being philosopherPos + (numPhilosophers - 1)) % numPhilosophers>. We do %
				 * here because we don't have to go overboard
				 * for the number of philosophers. philosopherPos is you, if you are hungry, and
				 * your neighbors (left and right positions) are not eating
				 * then you are able to eat. Else, you would have to wait() your turn.
				 */

				if (states[(philosopherPos + 1) % numPhilosophers] != STATE.EATING
						&& states[(philosopherPos + (numPhilosophers - 1)) % numPhilosophers] != STATE.EATING
	...
	public synchronized void putDown(final int piTID) {
		...
		// Notifies the other threads, so that they may be able to see if they are able
		// to see the actions of their neighbors
		notifyAll();
	}
	...
	public synchronized void requestTalk() {
		if (is_talking) {
			try {
				...
				/**
				 * Wait until another philosopher is still talking, when he's done, you request
				 * to talk
				 */
				wait();
				requestTalk();
				...
				System.out.println(
						"A philosopher is currently speaking something very useful. Please wait to philosophy");
			}
	...
	public synchronized void endTalk() {
		...
		// Notify all the other threads that you are the one done talking, and now they
